import cv2                      # Camera access and drawing
import face_recognition         # Face detection + encoding library
import numpy as np              # Numerical operations
import pickle                   # Save/load face database
import os                       # File system checks

class FaceRecognitionModule:
    def __init__(self, db_path="face_db.pkl"):
        self.db_path = db_path
        # Real-world face width (cm) and camera focal length (for future distance use)
        self.KNOWN_FACE_WIDTH = 14.5
        self.FOCAL_LENGTH = 600
        self.load_db()           # Load saved faces if available
    ...
    def register_new_face(self, person_name, num_samples=8):
        # Capture multiple samples to improve recognition accuracy
        ...
        while collected < num_samples:
            ...
            for enc in encodings:
                ...
                collected += 1
                # Show progress while collecting samples
                cv2.putText(
                    frame, f"Saving {collected}/{num_samples}",
        ...
        # Store average encoding for stability
        if temp_encodings:
    ...
    def identify_face(self, encoding):
        # Match face encoding against known database
        ...
        # Threshold controls strictness of recognition
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for loc, enc in zip(locs, encs):
            ...
            cv2.rectangle(
                frame, (loc[3], loc[0]), (loc[1], loc[2]),
                (0, 255, 0), 2
            )
            cv2.putText(
                frame, name, (loc[3], loc[0] - 10),
        ...
        if key == ord('q'):
            ...
            # Pause recognition to register a new face
            cap.release()
            cv2.destroyAllWindows()
